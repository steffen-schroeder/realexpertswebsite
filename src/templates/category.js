...
import CategoryPostTeaser from '../components/CategoryPostTeaser';
    ...
    return (
      <Layout>
        <section className="blog">
          <Helmet title={`Category | ${data.settings.global.title}`} link={[
        ...
        <div>
          ...
          <div className="top-posts">
            {topPosts.map((topPost, key) => (
              <CategoryPostTeaser key={topPost.id} post={topPost} type={key === 0 ? 'featured' : 'top'}/>
            ))}
          </div>
        </div>
        }
          ...
          <div className="all-posts">
            {posts.map(({node: post}) => (
              <CategoryPostTeaser key={post.id} post={post} type='normal'/>
            ))}
          </div>
...
export const pageQuery = graphql`
    query CategoryIndexQuery($skip: Int!, $limit: Int!) {
